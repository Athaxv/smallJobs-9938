...
import { getFallbackAnalyze } from '@template/web/ai-prompts';

import { Colors, Spacing, FontSize, Radius, Font } from '../../lib/theme';

import { aiApi, type AIQuestion } from '../../lib/api';
...
function goToPreview(
  ...
  router.replace({
    ...
    params: {
      ...
      ...(known ? { known: JSON.stringify(known) } : {}),
...
export default function AIFollowupScreen() {
  ...
  const [known, setKnown] = useState<Record<string, string> | undefined>();
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const data = await aiApi.analyze(request ?? '');
      ...
      if (data.ok) {
        ...
        if (data.complete || !data.questions?.length) {
          ...
          goToPreview(request ?? '', [], data.intent, data.known);
        ...
        setMessages([
          ...
          { id: 'ai-intro', sender: 'ai', text: 'Got it! One quick question if anything is missing.' },

          { id: 'ai-q0', sender: 'ai', text: data.questions[0].question },
  ...
  const useFallback = () => {
    ...
    const fallback = getFallbackAnalyze(request ?? '');

    if (fallback.intent) setIntent(fallback.intent);

    if (fallback.known) setKnown(fallback.known);



    if (fallback.complete || fallback.questions.length === 0) {

      goToPreview(request ?? '', [], fallback.intent, fallback.known);
    ...
    setFlow(fallback.questions);

    setMessages([

      { id: 'ai-intro', sender: 'ai', text: 'Got it! One quick question if anything is missing.' },

      { id: 'ai-q0', sender: 'ai', text: fallback.questions[0].question },
  ...
  const handleContinue = () => {
    ...
    goToPreview(request ?? '', answers, intent, known);
        ...
        <View style={styles.headerCenter}>
          ...
          <View>
            ...
            <Text style={styles.headerSub}>
              ...
              {loading ? 'Thinking...' : done ? 'Done!' : 'Almost ready...'}
