...
import {
  Alert, ImageBackground, StyleSheet, Text, TextInput, TouchableOpacity,
...
export default function Login() {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      animSpring(field1O, 1, 160),
      animSpring(field1Y, 0, 160),
      animSpring(field2O, 1, 220),
      animSpring(field2Y, 0, 220),
      animSpring(btnsO, 1, 300),
      animSpring(btnsY, 0, 300),
    ]).start();
  }, []);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Check onboarding status and route directly
      try {
  ...
  const handleGoogle = async () => {
    ...
    setError('');
    // Google sign-in not supported in this build
              ...
              <Animated.View style={[styles.inputGroup, { opacity: field2O, transform: [{ translateY: field2Y }] }]}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    Alert.alert('Coming soon', 'Password reset is not yet configured.');
                  }}
                >
...
const styles = StyleSheet.create({
  ...
  forgotText: { color: '#111111', fontSize: 13, fontFamily: Font.sansSemibold },
  btnPrimary: {
